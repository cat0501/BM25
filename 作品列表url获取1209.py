...
def extract_info(page_content,url):
    ...
    return {'name': name, 'encoded_href': encoded_href}
...
def crawl_info(url_list, driver):
    ...
    for i, url in enumerate(url_list, start=2):
        try:
            ...
            sheet.cell(row=i, column=2, value=info['encoded_href'])

        except Exception as e:
            print(f'在爬取URL：{url}时出现错误：{e}')

    workbook.save('poet_url1211_02.xlsx')
...
if __name__=='__main__':
    url_list = read_url_list_from_excel('D://learn//毕业设计//BM25-master//links_3.xlsx')  # 读取URL列表的文件路径
